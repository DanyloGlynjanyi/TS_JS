...
const radius = (r: number) => Math.PI * r * r

console.log(radius(6))
// - створити функцію яка обчислює та повертає площу циліндру висотою h, та радіутом r
const cylinder_area = (h: number, r: number) => 2 * Math.PI * r * (r + h)
...
const arrayLogger = (myArray: (boolean | string | number)[]): void => {
...
const creatorP = (text: string): void => {
...
const ulcreator2 = (text: string, count: number): void => {
    document.write(`<ul>`);
...
const ulCreator3 = (arr: (boolean | string | number)[]): void => {
    document.write(`<ul>`);
...
const minNumber = (arrOfNums: number[]): number => {
    let min = arrOfNums[0];
...
const sum = (arrOfNums: number[]): number => {
    let result: number = 0;
...
const exchange = (sum: number, currencies: ICur[], resultCurrency: string): string | number => {
...
const result2: string | number = exchange(10000, currencies, 'EUR');
